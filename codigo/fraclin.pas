...
{ Nota: Este archivo se puede proteger contra copias, colocando en SI la
  ----  constante VerifInt. En caso tal, compilar con la unidad UTIL.TPU.

        Este codigo se diferencia del de FRACLIND.PAS en que utiliza
        archivos independientes para cada tema de las instrucciones,
        haciendo mas rapido el acceso a la informacion (aunque requiere
        bastantes archivos individuales).
  ...
  Este codigo se hace publico con el UNICO interes de fomentar el trabajo 
  academico sobre fractales en nuestro medio, y NO para ser utilizado con
  fines comerciales. El autor solicita a quienes tengan acceso al codigo el
  no distribuir versiones compiladas del programa que incorporen cambios,
  sin previamente consultarlo.
...
{$EndIf}
  ...
  Crt, FracUni, FracExe, Printer {, Util} ;

CONST
  DirArchivoTextos = 'C:\TP\FRACTAL\';
  VerifInt = No;     { Activar proteccion contra copias (VerificarIntegridad) }
  PantIni = Si;

Var
  NombreArchivoTextos : String;
  MatNomTextos : Array [Instruc..Salir,1..12] Of String;
...
Procedure CrearMatNomTextos;
  Begin
    MatNomTextos[Instruc,2]:='Presenta'; MatNomTextos[Instruc,3]:='ManBasic';
    MatNomTextos[Instruc,4]:='Ayudas';   MatNomTextos[Instruc,5]:='Ventanas';
    MatNomTextos[Instruc,6]:='Demo';     MatNomTextos[Instruc,7]:='Ecuacion';
    MatNomTextos[Instruc,8]:='Pantalla'; MatNomTextos[Instruc,9]:='Graficar';
    MatNomTextos[Instruc,10]:='Archivo';

    MatNomTextos[Info,2]:='FrIntro';    MatNomTextos[Info,3]:='Math';
    MatNomTextos[Info,4]:='AlgoGraf';   MatNomTextos[Info,5]:='NotAutor';
    MatNomTextos[Info,6]:='Biblio';

    MatNomTextos[Suger,2]:='Prop';  MatNomTextos[Suger,3]:='Sol';
  End;      { Termina CrearMatNomTextos }
...
Procedure RutinasInicialesInfo;
  Begin
   { If VerifInt Then Verificar_Integridad;}
    SalirControl:=No;
    ParamDefectoInfo;
...
Const
  MaxPag              =  8;          { # maximo de paginas por seccion      }
...
Procedure CargueSeccion(PosiCont : LocalizacionCont; Var TotPag:Integer);
 ...
 Function TerminaSeccion:Boolean;   { Subproc de CargueSeccion }
   Var
     Lin         : String[80];
     Tam         : Integer;
   Begin
 ...
 Function TerminaPagina:Boolean;    { Subproc de CargueSeccion }
   Var
     Lin         : String[80];
     Tam         : Integer;
   Begin
 ...
 Begin                               { Comienza CargueSeccion      }
   ...
   With PosiCont do
     Begin
       NombreArchivoTextos:=MatNomTextos[Ventana,Renglon];
       NombreArchivoTextos:=DirArchivoTextos+NombreArchivoTextos+'.TXT';
       TituloABuscar:=TextosC[Ventana,Renglon];
   ...
   Until TextoPrueba=TituloABuscar;
   CuenPag:=1;
